...
module HSBC
  ...
  class Parser < ::Parser
    ...
    def initialize(csv_file)
      super(csv_file, HSBC::VendorMap.new)
    end
  ...
  class Downloader
    ...
    def login
      @driver.find_element(:name, 'userid').value = ENV['HSBC_USER']
      @driver.click_and_wait_for :method => :xpath, :expression => '//a/img[@alt="Go"]/..',
        :predicate => lambda { @driver.exists? :name, 'RCCsubmit' }
    end

    def answer_questions
      @driver.find_element(:name, 'memorableAnswer').value = ENV['HSBC_ANSWER']
      ...
      @driver.click_and_wait_for :method => :name, :expression => 'RCCsubmit',
        :predicate => lambda { @driver.exists? :link_text, ENV['HSBC_ACCT_NUMBER'] }
    end

    def goto_account
      @driver.click_and_wait_for :method => :link_text, :expression => ENV['HSBC_ACCT_NUMBER'],
